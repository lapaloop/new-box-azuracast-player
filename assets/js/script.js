...
function fetchData() {
	fetch(apiUrl + "/api/nowplaying")
		.then(
			(e) => (
				e.ok || checkError("Failed to load API data", () => location.reload()),
				e.json()
			)
		)
		.then((data) => {
...
function processData() {
  ...
  <li>
    ...
    <button class="music-item ${
      ...
      <img src="${stationsData[i].imgBrand}" width="800" height="800" alt="${
			stationsData[i].title
		} Album Poster"
        class="img-cover">
		...
		Array.isArray(d) && d.length > 0
			? d.forEach(async (b) => {
                    ...
                    <div class="ml-3 flex-grow">
                        <p class="text-2xl font-bold text-white text-left">${
													n.title
												}</p>
                        <p class="text-medium text-gray-400 text-left">By: ${
													n.artist
												}</p>
                        <p class="text-medium text-gray-400 text-left">From: ${
													n.album
												}</p>
                        <p class="text-xs text-gray-900 mt-1 text-left">${setTime(
													getTime(frDate)
												)}</p>
                    </div>
                `),
